...
import com.example.geektrust.model.EachJourneyChargeNeeded;
...
public class CardExecutionServiceImpl implements CardExecutionService {
	...
	public void processCheckIn(List<String> tokens) {
		...
		EachJourneyChargeNeeded journeyCharge = updateBalance(checkedIn.getMetroCardNumber(), checkedIn.getCharge());
	...
	public List<StationStatistics> calcEachStationStatistics(String fromStation,Map<String,List<PassengerCheckIn>> passengerAtStation) {
		...
		for(Map.Entry<String, List<PassengerCheckIn>> current: passengerAtStation.entrySet()) {
			...
			for(PassengerCheckIn passenger: current.getValue()) {
				...
				statistics.setTotalCharge(statistics.getTotalCharge()+passenger.getJourneyCharge().getCostOfOverallJourney());
				statistics.setTotalDiscount(statistics.getTotalDiscount()+passenger.getJourneyCharge().getTotalDiscount());
			}
	...
	public EachJourneyChargeNeeded updateBalance(String id, int charge) {
		...
		EachJourneyChargeNeeded journeyCharge=new EachJourneyChargeNeeded(discount,totalAmountCollected);
